...
def collect_report() -> VerificationReport:
    ...
    base_report = VerificationReport(
        simulacrax_backend=os.environ.get("SIMULACRAX_BACKEND"),
    ...
    try:
        ...
        return _with_error(base_report, error=str(exc))
    ...
    return VerificationReport(
        simulacrax_backend=base_report.simulacrax_backend,
...
def _with_error(report: VerificationReport, **overrides: Any) -> VerificationReport:
    """Return a copy of a frozen report with the given fields replaced."""
    values = asdict(report)
    values["devices"] = list(report.devices)
...
def render_human_report(report: VerificationReport) -> str:
    ...
    lines = [
        "Simulacrax JAX backend verification",
        f"Platform: {report.platform}",
        f"Python: {report.python}",
        f"Configured Simulacrax backend: {report.simulacrax_backend or 'unset'}",
    ...
    lines.append(
        "Note: Simulacrax leaves JAX_PLATFORMS unset by default so JAX can pick"
