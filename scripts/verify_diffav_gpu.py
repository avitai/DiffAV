...
def collect_report() -> VerificationReport:
    ...
    base_report = VerificationReport(
        diffav_backend=os.environ.get("DIFFAV_BACKEND"),
    ...
    return VerificationReport(
        diffav_backend=base_report.diffav_backend,
...
def render_human_report(report: VerificationReport) -> str:
    ...
    lines = [
        "DiffAV JAX backend verification",
        f"Platform: {report.platform}",
        f"Python: {report.python}",
        f"Configured DiffAV backend: {report.diffav_backend or 'unset'}",
    ...
    lines.append(
        "Note: DiffAV leaves JAX_PLATFORMS unset by default so JAX can pick"
