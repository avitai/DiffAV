...
Requirements:
    - jupytext: Install with `uv pip install jupytext` or `pip install jupytext`
...
def _comparable_cells(path: Path) -> list[tuple[str, str]]:
    """Parse a py/ipynb file into comparable (cell_type, source) tuples.

    Empty cells are dropped and sources stripped, so representation-only
    differences (trailing whitespace, empty trailing cells) do not count as
    drift; any real content change does.

    Args:
        path: Percent-format script or notebook.

    Returns:
        Ordered list of (cell_type, stripped_source) pairs.
    """
    import jupytext  # noqa: PLC0415 — optional dependency, imported at use

    notebook = jupytext.read(path)
    return [(cell.cell_type, cell.source.strip()) for cell in notebook.cells if cell.source.strip()]


def validate_sync(directory: Path, verbose: bool = False) -> tuple[int, int, int]:
    """Validate that .py and .ipynb pairs carry identical cell content.

    Read-only: unlike ``sync``, this never rewrites either file. A pair is
    synced only when the parsed cell sequence of the script matches the
    notebook cell-for-cell — pairing metadata alone does not count.
    ...
    py_files = [f for f in sorted(directory.rglob("*.py")) if f.name != "__init__.py"]

    print(f"Validating {len(py_files)} Python file(s) (content-level)")
    ...
    for py_file in py_files:
        ...
        try:
            py_cells = _comparable_cells(py_file)
            nb_cells = _comparable_cells(nb_file)
        except (ValueError, OSError) as e:
            print(f"  Error checking {py_file}: {e}")
            out_of_sync_count += 1
            continue

        if py_cells == nb_cells:
            if verbose:
                print(f"  Synced: {py_file}")
            synced_count += 1
        else:
            first_diff = next(
                (i for i, (a, b) in enumerate(zip(py_cells, nb_cells)) if a != b),
                min(len(py_cells), len(nb_cells)),
            )
            print(
                f"  Out of sync: {py_file} (first differing cell #{first_diff}; "
                f"run: uv run python scripts/jupytext_converter.py sync {py_file})"
            )
            out_of_sync_count += 1
