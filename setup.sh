...
MANAGED_ENV_FILE=".simulacrax.env"
...
usage() {
    cat <<'EOF'
Simulacrax development environment setup
...
Options:
  ...
  --force-clean                      Remove .venv, the generated .simulacrax.env, and repo-local test artifacts
  --dry-run                          Print the resolved backend and uv commands without changing files
  --help, -h                         Show this help

Notes:
  - Simulacrax uses uv for all repo-maintained setup and test workflows.
  ...
  - uv.lock is never deleted or regenerated by this script.
  - The generated backend file is .simulacrax.env. User-owned .env is not modified.
EOF
}
...
cat <<EOF
Simulacrax environment synced.
Resolved backend: $BACKEND
Managed backend file: $MANAGED_ENV_FILE

Next steps:
  source ./activate.sh
  uv run python scripts/verify_simulacrax_gpu.py
EOF
