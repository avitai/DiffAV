...
usage() {
    cat <<'EOF'
DiffAV development environment setup
...
Options:
  ...
  --force-clean                      Remove .venv, the generated .diffav.env, and repo-local test artifacts
  --dry-run                          Print the resolved backend and uv commands without changing files
  --help, -h                         Show this help

Notes:
  - DiffAV uses uv for all repo-maintained setup and test workflows.
  ...
  - The generated backend file is .diffav.env. User-owned .env is not modified.
EOF
}
...
cat <<EOF
DiffAV environment synced.
Resolved backend: $BACKEND
Managed backend file: $MANAGED_ENV_FILE

Next steps:
  source ./activate.sh
  uv run python scripts/verify_diffav_gpu.py
EOF
