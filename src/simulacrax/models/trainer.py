...
class TrainerConfig:
    ...
    num_epochs: int = 100
    physics_config: SimulacraxPhysicsConfig | None = None
...
class TrajectoryTrainer:
    ...
    :class:`TimingCollector`, and simulacrax's :class:`SimulacraxPhysicsLoss`
    and :class:`SimulacraxCheckpointManager` to implement a training loop
    ...
    def __init__(
        ...
        *,
        ...
        self._physics: SimulacraxPhysicsLoss | None = None
        if config.physics_config is not None:
            self._physics = SimulacraxPhysicsLoss(config.physics_config)

        # Checkpoint manager (optional)
        self._checkpoint: SimulacraxCheckpointManager | None = None
        if config.checkpoint_config is not None:
            self._checkpoint = SimulacraxCheckpointManager(config.checkpoint_config)
    ...
    @property
    def checkpoint_manager(self) -> SimulacraxCheckpointManager | None:
